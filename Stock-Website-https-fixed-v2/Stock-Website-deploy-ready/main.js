...
let hasShownApiConfigWarning = false;

function getApiOrigin() {
  return normalizeApiOrigin(window.__API_ORIGIN__ || localStorage.getItem("apiOrigin") || "");
}

function isLikelyStaticAwsFrontendHost() {
  const host = window.location.hostname || "";
  return host.includes("amazonaws.com") || host.includes("cloudfront.net");
}

function getApiConfigError(path = "") {
  const apiOrigin = getApiOrigin();

  if (window.location.protocol === "https:" && apiOrigin.startsWith("http://")) {
    return new Error(
      `Mixed content blocked: this site is HTTPS but the backend is HTTP (${apiOrigin}). Use a public HTTPS backend URL in config.js or localStorage.apiOrigin.`
    );
  }

  if (!apiOrigin && path.startsWith("/api/") && isLikelyStaticAwsFrontendHost()) {
    return new Error(
      "Backend API origin is not configured. Set config.js or localStorage.apiOrigin to your deployed backend HTTPS URL."
    );
  }
...
function setApiOrigin(origin) {
  ...
  if (normalizedOrigin) {
    localStorage.setItem("apiOrigin", normalizedOrigin);
  }
}

function clearApiOrigin() {
...
async function apiFetch(path, options = {}) {
  ...
  const configError = getApiConfigError(path);
  if (configError) {
    if (!hasShownApiConfigWarning) {
      hasShownApiConfigWarning = true;
  ...
  try {
    ...
    const apiOrigin = getApiOrigin();
    const prefix = apiOrigin ? `API request to ${apiOrigin} failed.` : "API request failed.";
    throw new Error(`${prefix} Open DevTools > Console/Network for the browser error details.`);
  }
}

function getErrorMessage(data, fallback = "Request failed") {
  if (Array.isArray(data?.details) && data.details.length) {
    return data.details.map((item) => item?.message || "Validation error").join(" | ");
  }
...
function pageNeedsStocks() {
  return Boolean(
    elements.stockList ||
    elements.tableBody ||
    elements.adminTableBody ||
    requireElement("stocksTableBody")
  );
}

async function loadStocks() {
  if (!pageNeedsStocks()) return;
  ...
  elements.addStockForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(err?.error || err?.message || "Failed to save stock");
      }
...
function pageNeedsMarketHours() {
  return Boolean(
    requireElement("marketHoursTable") ||
    requireElement("holidayHoursList") ||
    requireElement("marketCalendar") ||
    requireElement("calendarMonthLabel")
  );
}

async function loadMarketHours() {
  if (!pageNeedsMarketHours()) return;
...
function initRegisterPage() {
  ...
  registerForm.addEventListener("submit", async (event) => {
    ...
    const username = requireElement("username")?.value?.trim();
...
function initLoginPage() {
  ...
  loginForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const data = await parseJsonSafely(response);
      if (!response.ok) {
        throw new Error(getErrorMessage(data, "Login failed. Check your credentials."));
      }
  ...
  if (confirmSellBtn) {
    confirmSellBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const data = await parseJsonSafely(response);
        if (!response.ok) {
          throw new Error(data?.error || data?.message || "Sell failed.");
        }
